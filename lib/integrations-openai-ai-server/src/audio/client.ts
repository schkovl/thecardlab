...
function getAudioOpenAIClient(): OpenAI {
...
let _audioOpenai: OpenAI | undefined;

export const openai = new Proxy({} as OpenAI, {
  get(_target, prop) {
    if (!_audioOpenai) _audioOpenai = getAudioOpenAIClient();
    return (_audioOpenai as unknown as Record<string | symbol, unknown>)[prop];
  },
});
